...
#define NB_RESET_GPIO 					GPIOA
#define NB_RESET_PIN						GPIO_Pin_8
#define NB_RESET_OFF						GPIO_ResetBits(NB_RESET_GPIO, NB_RESET_PIN)
...
VOID * main_nb95(UINT32 uwArg)
{
	...
	while (1)
	{
		...
		ret 			= neul_bc95_set_cdpserver("218.4.33.72"); //("180.101.147.208");//
		if (ret < 0)
		{
...
UINT32 creat_maintask(const char * taskname, TSK_ENTRY_FUNC fnTask, UINT32 stackisize,
	 UINT32 uwArg, UINT16 prior)
{
	UINT32		uwRet = LOS_OK;
	UINT32		handle;
